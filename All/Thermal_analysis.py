...
def thermal_describtion_glass(glass_name,temperature_list,formula_number,formula_name1,A0,A1,A2,A3,A4,A5,A6,A7,A8,A9):
    ...
    while True:
        response = input("Do you want to compare with different wavelengths of same glass type for ABSOLUTE REFRACTIVE INDEX WITH TEMPERATURE(y/n)  :  ")
        if response == 'y':
    ...
    wavelength = float(input("\nEnter the wavelength in nm to find refractive index at given temperature  :  "))
    wavelength = wavelength/1000
    ...
    n_air_catT0 = (1.0000  +  (((n_air_15 - 1))/(1.0000 + 0.0034785*(T0 - 15))) )
    ...
    #print the refractive index at given temperature
    ...
    while True:
        response = input("Do you want to get comaparison between different wavelengths of same glass type for RELATIVE REFRACTIVE INDEX CHANGE WITH TEMPERATURE  :  ")
        if response == 'y':
